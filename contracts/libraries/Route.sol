...
library Route {
        ...
        )
    {
        ...
        routeHelper.aggregatedPool = Structs.Amm(
            amms[routeHelper.sortedIndices[amms.length - 1]].x,
            amms[routeHelper.sortedIndices[amms.length - 1]].y
        );
        totalY = 0;
        ...
        for (uint256 j = amms.length - 1; j > 0; --j) {
            Structs.Amm memory nextBestAmm = amms[
                routeHelper.sortedIndices[j - 1]
            ];
            ...
            if (routeHelper.deltaX >= amountOfX) {
                ...
                deltaXWorst = SharedFunctions.howMuchXToSpendToLevelAmms(
                    routeHelper.aggregatedPool,
                    amms[routeHelper.sortedIndices[0]]
                );
                if (deltaXWorst == 0) {
                    shouldArbitrage = false;
                }
            ...
            for (uint256 k = 0; k < amms.length - j; ++k) {
                uint256 yGain = SharedFunctions.quantityOfYForX(
                    ...
                    routeHelper.splits[k]
                );
                xToSendToAmms[
                    routeHelper.sortedIndices[
                        amms.length - routeHelper.splits.length + k
                    ]
                ] += routeHelper.splits[k];
                amms[
                    routeHelper.sortedIndices[
            ...
            (routeHelper.aggregatedPool.x, routeHelper.aggregatedPool.y) = (
                0,
                0
            );
            for (uint256 k = 0; k < amms.length - j; ++k) {
                routeHelper.aggregatedPool.x += amms[
                    routeHelper.sortedIndices[amms.length - 1 - k]
                ].x;
                routeHelper.aggregatedPool.y += amms[
                    routeHelper.sortedIndices[amms.length - 1 - k]
                ].y;
            }
