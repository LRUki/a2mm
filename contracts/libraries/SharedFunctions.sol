...
library SharedFunctions {
    ...
    function _quantityOfYForX(
        ...
        uint256 amountInWithFee = dx * 997;
        uint256 numerator = amountInWithFee * y;
        uint256 denominator = x * 1000 + amountInWithFee;
        amountOut = numerator / denominator;
    }

    function quantityOfYForX(Structs.Amm memory amm, uint256 dx)
        public
        pure
        returns (uint256)
    {
        return _quantityOfYForX(amm.x, amm.y, dx);
    }

    function quantityOfXForY(Structs.Amm memory amm, uint256 dy)
        public
        pure
        returns (uint256)
    {
        return _quantityOfYForX(amm.y, amm.x, dy);
    }

    function _howMuchToSpendToLevelAmms(
        uint256 t11,
        uint256 t12,
        uint256 t21,
        uint256 t22
        ...
        require(
            ...
            "Liquidity must be more than 0."
        );
        uint256 left = sqrt(t11 * t22) *
            sqrt((t11 * t22 * 2257) / 1_000_000_000 + t12 * t21);
    ...
    function howMuchXToSpendToLevelAmms(
        Structs.Amm memory betterAmm,
        Structs.Amm memory worseAmm
    ...
    function howMuchXToSpendToLevelAmmsWrapper(
        uint256[2] memory betterAmmArray,
        uint256[2] memory worseAmmArray
    ) public view returns (uint256) {
        Structs.Amm memory betterAmm = Structs.Amm(
            betterAmmArray[0],
            betterAmmArray[1]
        );
        Structs.Amm memory worseAmm = Structs.Amm(
            worseAmmArray[0],
            worseAmmArray[1]
        );
    ...
    function howMuchYToSpendToLevelAmms(
        Structs.Amm memory betterAmm,
        Structs.Amm memory worseAmm
    ) public view returns (uint256) {
        uint256 x1 = betterAmm.x;
        ...
        returns (uint256[] memory indices)
    {
        ...
        for (uint256 i = 1; i < n; ++i) {
            ...
            while (
                j > 0 &&
                amms[tmp].y * amms[indices[j - 1]].x <
                amms[indices[j - 1]].y * amms[tmp].x
            ) {
    ...
    function howToSplitRoutingOnLeveledAmms(
        ...
        uint256 delta
    ) public pure returns (uint256[] memory splits) {
